...
func (service *userService) Register(userInput user.Core) error {
	...
	errRegister := service.userData.Register(userInput)
	return errRegister
}
...
// Profil implements user.UserServiceInterface.
func (service *userService) Profil(id int) ([]user.Core, error) {
	data, err := service.userData.Profil(id)
	return data, err
}

// UpdatedProfil implements user.UserServiceInterface.
func (service *userService) UpdatedProfil(id int, userInput user.Core) error {
	// Mengatur ulang validator
	validate := validator.New()
	UpdatedProfil := user.UpdatedProfil{
		User_name: userInput.User_name,
		Phone:     userInput.Phone,
		Email:     userInput.Email,
		Password:  userInput.Password,
	}
	errValidate := validate.Struct(UpdatedProfil)
	if errValidate != nil {
		return errValidate
	}
	...
	errUpdate := service.userData.UpdatedProfil(id, userInput)
	return errUpdate
}
